param ([string] $DOWNLOAD_PATH, $ZIP_FILE_URL, $ZIP_FILE_NAME, $EXTRACT_PATH, $CHECK_PATH)
If(test-path $CHECK_PATH )
{
    Write-Output "extract path avaliable, stop extract zip file."
}
else {
    ...
    Get-ChildItem $DOWNLOAD_PATH -Filter $ZIP_FILE_NAME  | Expand-Archive -DestinationPath $EXTRACT_PATH
}
